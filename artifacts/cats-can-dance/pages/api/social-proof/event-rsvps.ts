/**
 * GET /api/social-proof/event-rsvps?slug=<event_slug>
 * Returns RSVP count for a specific event.
 */
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const r = await fetch(
      `${SB}/rest/v1/event_rsvps?event_slug=eq.${encodeURIComponent(slug)}&select=id&limit=0`,
