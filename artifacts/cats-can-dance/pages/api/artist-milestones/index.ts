/**
 * /api/artist-milestones
 *
 * GET    ?artist_slug=<slug>  — list milestones
 * POST   { ... }              — create
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    const filters: Record<string, string> = { ...ord("date") };
    ...
    return res.json(await get("artist_milestones", pq(filters)) ?? []);
  }

  const now = new Date().toISOString();

  if (req.method === "POST") {
    const { ok, data } = await ins("artist_milestones", { ...req.body, created_at: now, updated_at: now });
  ...
  if (req.method === "PATCH") {
    ...
    const { ok, data } = await patch("artist_milestones", pq(eqf("id", id)), { ...req.body, updated_at: now });
  ...
  if (req.method === "DELETE") {
    const id = (req.query.id ?? req.body?.id) as string;
    if (!id) return res.status(400).json({ error: "id required" });
    await del("artist_milestones", pq(eqf("id", id)));
