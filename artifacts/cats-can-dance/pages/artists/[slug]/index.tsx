 ...
 * Supabase on every request — no self-HTTP calls, no client double-fetch.
 *
 ...
 *   Result: loading spinner visible on every visit, slow ISR cold starts
 *
 * Now (correct pattern):
 *   1. getServerSideProps → getArtistFullProfile(slug)           ← direct DB, fast
 ...
 *
 * SEO: bio, genres, gigs, connections all in HTML source → Google indexes everything.
 * Social cards: artist photo + bio in og:image/og:description on share.
 *
 * Caching: 2-min Vercel Edge Cache (artists update occasionally).
 */
...
export const getServerSideProps: GetServerSideProps<Props> = async ({
  ...
  // Single function — fetches artist + all relations in parallel
  const profile = await getArtistFullProfile(slug);

  // Artist not found → 404 page
  if (!profile) {
