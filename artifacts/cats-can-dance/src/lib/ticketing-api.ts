 ...
 * Uses credentials: "include" so Clerk session cookies are forwarded.
 */
...
export const initiateTransfer = (qrToken: string, body: { to_email: string; to_name?: string }) =>
  apiFetch(`/tickets/${qrToken}/transfer`, { method: "POST", body: JSON.stringify(body) });
...
export const cancelTransfer = (claimToken: string) =>
  apiFetch(`/transfers/${claimToken}/cancel`, { method: "POST", body: JSON.stringify({}) });
...
export const getPromoterMe = () => apiFetch("/promoter/me");

export const getPromoterEvents = () => apiFetch("/promoter/events");

export const createEventTicketing = (body: Record<string, unknown>) =>
  apiFetch("/promoter/events", { method: "POST", body: JSON.stringify(body) });

export const updateEventTicketing = (slug: string, body: Record<string, unknown>) =>
  apiFetch(`/promoter/events/${slug}`, { method: "PATCH", body: JSON.stringify(body) });

export const createTier = (slug: string, body: Record<string, unknown>) =>
  apiFetch(`/promoter/events/${slug}/tiers`, { method: "POST", body: JSON.stringify(body) });

export const updateTier = (tierId: string, body: Record<string, unknown>) =>
  apiFetch(`/promoter/tiers/${tierId}`, { method: "PATCH", body: JSON.stringify(body) });

export const deleteTier = (tierId: string) =>
  apiFetch(`/promoter/tiers/${tierId}`, { method: "DELETE" });

export const getPromoterOrders = (slug: string) =>
  apiFetch(`/promoter/events/${slug}/orders`);

export const getPromoterRsvps = (slug: string) =>
  apiFetch(`/promoter/events/${slug}/rsvps`);

export const approveRsvp = (rsvpId: string, body?: { tier_id?: string }) =>
  apiFetch(`/promoter/rsvps/${rsvpId}/approve`, { method: "POST", body: JSON.stringify(body ?? {}) });

export const declineRsvp = (rsvpId: string, body?: { reason?: string }) =>
  apiFetch(`/promoter/rsvps/${rsvpId}/decline`, { method: "POST", body: JSON.stringify(body ?? {}) });

export const doorCheckin = (body: { qr_token: string; gate?: string }) =>
  apiFetch("/promoter/checkin", { method: "POST", body: JSON.stringify(body) });
...
export const adminLinkPromoterUser = (body: { clerk_user_id: string; promoter_id: string; email: string; display_name?: string; role?: string }) =>
